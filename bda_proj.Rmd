...
qt$model.include <- c(1,1,1,0,1,1,0,0,1)
qt <- qt[, c(1,6,5,3,2,4,7,8)]
```
...
```{r, echo = FALSE}
bc.df <- data.frame(rbind(summary(qt$body.count[qt$model.include == 1])), 
                    Var = var(qt$body.count[qt$model.include == 1]), 
                    check.names = FALSE)
...
* Filter conditions:
    ...
    - genre: crime, drama, thriller, action, western, mystery
    - director: not Quentin
* Sample size: 243

Kill rate per hour broken down by genre (this is a rough summary as many movies will fall into multiple categories):  
```{r, echo = FALSE}
qt.all <- qt
## Remove Inglorious Bastards
qt <- qt[qt$model.include == 1, ]
...
for (i in seq_along(dc.g)) {
  dc$Genre_Match[i] <- all(dc.g[[i]] %in% qt.g)
}
...
genre.sum <- data.frame(N = c(sum(dc.filt$Genre_Crime), sum(dc.filt$Genre_Drama), sum(dc.filt$Genre_Thriller), sum(dc.filt$Genre_Action),
                 sum(dc.filt$Genre_Mystery), sum(dc.filt$Genre_Western)), 
           rbind(summary(dc.filt$Kill_Rate[dc.filt$Genre_Crime == 1]), summary(dc.filt$Kill_Rate[dc.filt$Genre_Drama == 1]),
                 summary(dc.filt$Kill_Rate[dc.filt$Genre_Thriller == 1]),
                 summary(dc.filt$Kill_Rate[dc.filt$Genre_Action == 1]), summary(dc.filt$Kill_Rate[dc.filt$Genre_Mystery == 1]), 
                 summary(dc.filt$Kill_Rate[dc.filt$Genre_Western == 1])), 
           row.names = qt.g, 
           check.names = FALSE)
...
# II. Model - Excludes 3 High Count Movies
...
From the density histogram above, the density appears to be monotonically decreasing in the interval $(0,\infty)$ suggesting a shape parameter $\alpha = 1$. The vast majority of the data is less than 100, so we solve the following equation for $\beta$ taking $\alpha = 1$:
$$\begin{align*}
0.99 & = \int\limits_0^{100} \beta e^{-\beta\theta} d\theta \\
& = 1 - e^{-100\beta} \\
\implies \beta & = \frac{-1}{100} \log(0.01) \\
& = 0.046
\end{align*}$$
Therefore, the conjugate prior distribution of $\theta$ is:
$$p(\theta ~|~ \alpha = 1, \beta = 0.046) \propto e^{-(0.046)\cdot\theta}$$

### Posterior Distribution  

In general, the posterior distribution is Gamma$\left(\alpha + \sum\limits_{i=1}^n y_i, \beta + \sum\limits_{i=1}^n t_i\right)$. Thus, the posterior distribution of $\theta$ is Gamma(60, 13.78)
...
post.preds <- mapply(dnbinom, x = 0:35, MoreArgs = list(size = rep(alpha.post, length(t.new)), prob = (beta.ppd/(beta.ppd + 1))))
...
plot(1, 1, ann = FALSE, axes = FALSE, type = "n", lty = 3, col = "grey50", bty = "n", xlim = c(0, 35), ylim = c(0, 0.2))
for (i in 1:4) {
...
axis(2, seq.int(0,0.2,0.02), col = "gray50")
...
#text(x.modes + 3, modes + 0.04, labels = round(modes, 3), pos = 4)
#arrows(x.modes - 1, modes, x.modes + 3, modes + 0.04, length = 0.1, code = 1)
