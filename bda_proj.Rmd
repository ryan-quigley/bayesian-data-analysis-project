...
$$p(\theta) \propto \theta^{\alpha}e^{-\beta\cdot\theta}$$
...
The parameters for the Gamma distribution were determined by solving the system of equations generated by setting the mode equal to 8.22 and by observing that approximately 99% of the data is in the range (0,100).  
...
## Plot theoretical gamma distribution
curve(dgamma(x, shape = alpha.m2, rate = beta.m2), from = 0, to = 250, add = TRUE, lty = 2, col = "slateblue")
#legend("top", legend = paste(c("Mean =", "Mode =", "Variance ="), c(g.mean, g.mode, g.var)), bty = "n")
...
The parameters for the Gamma distribution were determined by solving the system of equations generated by setting the mode equal to 31.51 and by observing that approximately 99% of the data is in the range (0,100).  
...
The parameters for the Gamma distribution were determined by solving the system of equations generated by setting the mode equal to 43.83 and by observing that approximately 99% of the data is in the range (0,400).  


Model |  Prior: $p(\theta)$ | Named
------|---------------------|---------
 1 | $\propto \theta^{1.46 - 1}e^{-(0.056)\theta}$ | Gamma(1.46, 0.056)
 2 | $\propto \theta^{4.33 - 1}e^{-(0.106)\theta}$ | Gamma(4.33, 0.106)
 ...
 * Hierachichal model
 * Negative binomial in order to handle overdispersion
 * Poisson regression
