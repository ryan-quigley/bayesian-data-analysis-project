...
Quentin Tarantino’s moves are famous (or notorious) for violent death and massacre. In this project, we use Bayesian method to analyze and predict the body count in Quentin Tarantino’s next movie. We construct our prior belief on killed rate per hour based on 2xx movies with similar filming style, and update our belief with actual body count in all of his movies. In fact, we divide our study into 3 sub-models because we believe different movie genres should should have different death rates. At the end, our results show that for the same movie duration, if Quentin Tarantino’s next movie is about a war, the body count could be as many as 38 times a non-war related movie.
...
names(qt.data.sum) <- c("Movie", "Genre", "Year", "Rating", "Body Count", "Hours", "Kill Rate/Hr" )
h8.gsplit <- unlist(strsplit(qt.data.sum[9,2], ", ", fixed = TRUE))
kbv2.gsplit <- unlist(strsplit(qt.data.sum[5,2], ", ", fixed = TRUE))
qt.data.sum[5,2] <- paste(kbv2.gsplit[1:2], collapse = ", ")
qt.data.sum[9,2] <- paste(h8.gsplit[1:3], collapse = ", ")
...
The body count numbers in these three movies are unsually large and require approximation for one main reason: each contains a single scene of unimaginable (to most people except Tarantino) violence that dramatically increasese the death toll. In light of this information, we find it impossible to judge exchangeability for the entire nine movie filmography of Tarantino as both writer and director. We belive Gelman et. al would support our judgement given their published opinion on the matter,  

> "In practice, ignorance implies exchangeability. Generally, the less we know about a problem, the more confidently we can make claims of exchangeability. (This is not, we hasten to add, a good reason to limit our knowledge of a problem before embarking on statistical analysis!)"  
> - Gelman et. al BDA


The following table summarizes the model components, and provides two summary statistics for $\theta$ (Note: $\Gamma(\alpha, \beta) = \text{Gamma}(\alpha, \beta)$).  
...
The following table summarizes the kill rate per hour of the subset of the historical data used for each model:
...
ppc.df <- data.frame(c("Minimum", "Maximum", "Range", "Interquartile Range", "Mean", "Variance", "Var/Mean Ratio"),
                     p = round(c(pval.min, pval.max, pval.range, pval.IQR, pval.mean, pval.var, pval.ratio), 2), 
                     m = c(">=", 
                           "<=", 
                           "<=", 
                           "<=", 
                           ...
                           "<="), 
                     row.names = NULL)
names(ppc.df) <- c("Test Quantity", "P-Value", "Calculation Method")
knitr::kable(ppc.df, align = "l")
```
...
```{r, echo = FALSE}
par(mfrow = c(1,2))
h <- hist(sim.range, right = FALSE, freq = FALSE, ann = FALSE, col = "#1f78b4", border = "#a6cee3", xlim = c(0,30), ylim = c(0,0.2))
lines(rep(obs.range, 2), c(0, max(h$density)), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval.range, 2)), side = 3, cex = 0.75)
text(obs.range, max(h$density), obs.range, pos = 3)
title(main = "Full", cex.main = 1, line = 1)
title(xlab = expression(paste("T(y, ", theta, ")", sep = "")), ylab = "Density")

h <- hist(sim.IQRange, right = FALSE, freq = FALSE, ann = FALSE, col = "#1f78b4", border = "#a6cee3", xlim = c(0,20), ylim = c(0,0.4))
lines(rep(obs.IQRange, 2), c(0, max(h$density)), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval.IQR, 2)), side = 3, cex = 0.75)
text(obs.IQRange, max(h$density), obs.IQRange, pos = 3)
title(main = "IQR", cex.main = 1, line = 1)
title(xlab = expression(paste("T(y, ", theta, ")", sep = "")), ylab = "Density")
par(mfrow = c(1,1))
```
...
```{r, echo = FALSE}
par(mfrow = c(1,2))
h <- hist(sim.mean, right = FALSE, freq = FALSE, ann = FALSE, col = "#1f78b4", border = "#a6cee3", xlim = c(0,25), ylim = c(0,0.2))
lines(rep(obs.mean, 2), c(0, max(h$density)), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval.mean, 2)), side = 3, cex = 0.75)
text(obs.mean, max(h$density), round(obs.mean, 2), pos = 3)
title(main = "Mean", cex.main = 1, line = 1)
title(xlab = expression(paste("T(y, ", theta, ")", sep = "")), ylab = "Density")

h <- hist(sim.var, right = FALSE, freq = FALSE, ann = FALSE, col = "#1f78b4", border = "#a6cee3", xlim = c(0,60), ylim = c(0,0.1))
lines(rep(obs.var, 2), c(0, max(h$density)), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval.var, 2)), side = 3, cex = 0.75)
text(obs.var, max(h$density), round(obs.var, 2), pos = 3)
title(main = "Variance", cex.main = 1, line = 1)
title(xlab = expression(paste("T(y, ", theta, ")", sep = "")), ylab = "Density")
par(mfrow = c(1,1))
```
