...
for (i in seq_along(dc.g)) {
  dc$Genre_Match_M1[i] <- all(dc.g[[i]] %in% qt.g.m1)
  dc$Genre_Match_M2[i] <- all(dc.g[[i]] %in% qt.g.m2)
  dc$Genre_Match_M3[i] <- all(dc.g[[i]] %in% qt.g.m3)
}

## Filter death count dataset
dc.filt.m1 <- dc[dc$MPAA_Rating %in% c("R", "Unrated", "NR") & dc$Year > 1991 & dc$Genre_Match_M1 == TRUE, ]
dc.filt.m2 <- dc[dc$MPAA_Rating %in% c("R", "Unrated", "NR") & dc$Year > 1991 & dc$Genre_Match_M2 == TRUE, ]
dc.filt.m3 <- dc[dc$MPAA_Rating %in% c("R", "Unrated", "NR") & dc$Year > 1991 & dc$Genre_War == 1, ]
genre.sum <- data.frame(N = rbind(length(dc.filt.m1$Kill_Rate), length(dc.filt.m2$Kill_Rate), length(dc.filt.m3$Kill_Rate)), 
...
axis(1, at = seq.int(0, 200, 50), labels = NA)
axis(1, at = seq.int(0, 200, 100), lwd = 0, lwd.ticks = 0)
axis(2, at = seq.int(0, 0.06, 0.01))

## Plot theoretical gamma distribution
curve(dgamma(x, shape = alpha, rate = beta), from = 0, to = 200, add = TRUE, lty = 2, col = "slateblue")
...
d2 <- density(dc.filt.m2$Kill_Rate, kernel = "epanechnikov", bw = h, from = 0, to = 100)
plot(1,0.01, ann = FALSE, axes = FALSE,
     xlim = c(0, 100), ylim = c(0, 0.06), 
     bty = "n", type = "n")
hist(dc.filt.m2$Kill_Rate, freq = FALSE, axes = FALSE, ann = FALSE, add = TRUE, border = "grey75", col = "grey95")
lines(d2, col = "violetred", lwd = 1.5)
axis(1, at = seq.int(0, 100, 20))
axis(2, at = seq.int(0, 0.06, 0.01))
...
d3 <- density(dc.filt.m3$Kill_Rate, kernel = "epanechnikov", bw = h, from = 0, to = 350)
plot(1,0.01, ann = FALSE, axes = FALSE,
     xlim = c(0, 350), ylim = c(0, 0.02), 
     bty = "n", type = "n")
...
axis(1, at = seq.int(0, 350, 50), labels = NA)
axis(1, at = seq.int(0, 350, 100), lwd = 0, lwd.ticks = 0)
