...
The plots show the full posterior predictive distribution of each model for a sample of easily interpretable movie durations. For each model, the spread of the distribution increases with movie duration meaning our predictions for the next movie are less precise.
...
par(mfrow = c(1,2), cex.axis = 0.75, cex.main = 0.75, cex.lab = 0.75)
mltnom.check.m1 <- rmultinom(n = 10000, prob = rep(1/6, 6), size = sum(qt.m1$body.count))
stripchart(mltnom.check.m1, method = "stack", offset = 0.0008, pch = 20, 
           col = "violetred", at = 0, ann = FALSE, axes = FALSE, xlim = c(0,30), ylim = c(0,2))
axis(1, at = seq.int(0,30,5))
points(qt.m1$body.count, rep(0, 6), pch = 17, cex = 1, col = "black")
title(main = "Model 1", xlab = "Bin Counts")

mltnom.check.m2 <- rmultinom(n = 10000, prob = rep(1/2, 2), size = sum(qt.m2$body.count))
stripchart(mltnom.check.m2, method = "stack", offset = 0.004, pch = 20, 
           col = "violetred", at = 0, ann = FALSE, axes = FALSE, xlim = c(30, 90))
#axis(1, at = 30:30, labels = NA)
axis(1, at = seq.int(30,90,10))#, lwd = 0, lwd.tick = 0)
points(qt.m2$body.count, rep(0, 2), pch = 17, cex = 1, col = "black")
title(main = "Model 2", xlab = "Bin Counts")
par(mfrow = c(1,1))
```
