...
Summary of historical kill rate per hour data used to calculate the parameters or the conjugate prior distributions for each model:
...
genre.sum <- round(data.frame(1:3, rbind(length(dc.filt.m1$Kill_Rate), length(dc.filt.m2$Kill_Rate), length(dc.filt.m3$Kill_Rate)), 
                        ...
                        row.names = NULL), 2)
names(genre.sum) <- c("Model", "N", "Min.", "Q.1", "Median", "Mean", "Q.3", "Max.")
...
Model | Likelihood: $p(y ~\vert~ \theta)$ 
------|-----------------------------------
...
Data compiled by Randal Olson from the site: http://www.moviebodycounts.com/. See references for links to original dataset.
...
Model |  Posterior: $p(\theta ~\vert~ y)$ | Named
------|----------------------|---------
 1 | $\propto \theta^{60.46 - 1}e^{-13.79\cdot\theta}$ | Gamma(60.46, 13.79)
 2 | $\propto \theta^{128.13 - 1}e^{-4.66\cdot\theta}$ | Gamma(128.13, 4.66)
 3 | $\propto \theta^{398.015 - 1}e^{-2.57\cdot\theta}$ | Gamma(398.015, 2.57)


##### 99% Highest Posterior Density Interval for $\theta$
...
In this section we consider the effects of the choice of prior distribution on posterior inference. First we consider the effects of a noninformative prior instead of the gamma conjugate prior. The Jeffreys prior for $\theta$ follows from the Fisher information calculation below:  
$$\begin{align*}
...
h <- hist(yrep.m2, breaks = 100, right = FALSE, freq = FALSE, ann = FALSE, col = "#1f78b4", border = "#a6cee3", xlim = c(0,140), ylim = c(0,0.04))
lines(rep(obs.mean.m2, 2), c(0, max(h$density)), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval.mean.m2, 2)), side = 3, cex = 0.75)
text(obs.mean.m2, max(h$density), round(obs.mean.m2, 2), pos = 3)
title(main = "Model 2: Mean", line = 1)
title(xlab = expression(paste("T(y, ", theta, ")", sep = "")), ylab = "Density")
```
...
## Simulation set-up
m <- 500000
n.obs.m3 <- length(qt.m3$body.count)
theta.sim.m3 <- rgamma(m, shape = alpha.post.m3, rate = beta.post.m3)
t.sim.m3 <- qt.m3$hours
yrep.m3 <- round(mapply(rpois, n = n.obs.m3, lambda = theta.sim.m3*t.sim.m3))

h <- hist(yrep.m3, breaks = 100, right = FALSE, freq = FALSE, ann = FALSE, col = "#1f78b4", border = "#a6cee3", xlim = c(200,600), ylim = c(0,0.02))
lines(rep(qt.m3$body.count, 2), c(0, max(h$density)), col = "#e31a1c", lwd = 1.5)
text(qt.m3$body.count, max(h$density), qt.m3$body.count, pos = 3)
title(main = "Model 3", xlab = expression(paste("T(y, ", theta, ")", sep = "")), ylab = "Density")
```
