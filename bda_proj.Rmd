...
* Dataset does not contain movies with 0 deaths
* Filter conditions:
    - year: 1991-2013
    - rating: R, Unrated, or NR
    - genre: crime, drama, thriller, action, adventure, war, western, mystery
    - director: not Quentin
* Sample size: 244

Kill rate per hour broken down by genre (this is a rough summary as many movies with fall into multiple categories):  
```{r, echo = FALSE}
...
genre.sum <- data.frame(N = c(sum(dc.filt$Genre_Crime), sum(dc.filt$Genre_Drama), sum(dc.filt$Genre_Thriller), sum(dc.filt$Genre_Action),
                 sum(dc.filt$Genre_Adventure), sum(dc.filt$Genre_War), sum(dc.filt$Genre_Western), sum(dc.filt$Genre_Mystery)), 
           rbind(summary(dc.filt$Kill_Rate[dc.filt$Genre_Crime == 1]), summary(dc.filt$Kill_Rate[dc.filt$Genre_Drama == 1]), summary(dc.filt$Kill_Rate[dc.filt$Genre_Thriller == 1]),
                 summary(dc.filt$Kill_Rate[dc.filt$Genre_Action == 1]), summary(dc.filt$Kill_Rate[dc.filt$Genre_Adventure == 1]), summary(dc.filt$Kill_Rate[dc.filt$Genre_War == 1]),
                 summary(dc.filt$Kill_Rate[dc.filt$Genre_Western == 1]), summary(dc.filt$Kill_Rate[dc.filt$Genre_Mystery == 1])), 
           row.names = qt.g, 
           check.names = FALSE)
...
```{r, echo = FALSE}
war <- data.frame(rbind(summary(dc.filt$Kill_Rate[dc.filt$Genre_War == 0]), summary(dc.filt$Kill_Rate[dc.filt$Genre_War == 1])), row.names = c("Not war", "War"), check.names = FALSE)
knitr::kable(war, align = "l")
```

```{r, echo = FALSE}
qt.all <- qt
## Remove Inglorious Bastards
qt <- qt[-7, ]
```
---
...
### Likelihood (Excluding Inglorious Bastards)
...
& \propto \theta^{185} e^{ -18.33 \cdot \theta}
\end{align*}$$
...
d <- density(dc.filt$Kill_Rate[dc.filt$Genre_War == 0], kernel = "epanechnikov", bw = h, from = 0, to = 250)
plot(1,0.01, ann = FALSE, axes = FALSE,
     xlim = c(0, 250), ylim = c(0, 0.04), 
     bty = "n", type = "n")
hist(dc.filt$Kill_Rate[dc.filt$Genre_War == 0], breaks = 30, freq = FALSE, axes = FALSE, ann = FALSE, add = TRUE, border = "grey75", col = "grey95")
lines(d, col = "violetred", lwd = 1.5)
axis(1, at = seq.int(0, 250, 50), labels = NA)
axis(1, at = seq.int(0, 250, 100), lwd = 0, lwd.ticks = 0)
axis(2, at = seq.int(0, 0.04, 0.01))
...
curve(dgamma(x, shape = alpha, rate = beta), from = 0, to = 250, add = TRUE, lty = 2, col = "slateblue")
#legend("top", legend = paste(c("Mean =", "Mode =", "Variance ="), c(g.mean, g.mode, g.var)), bty = "n")
legend("top", legend = c("Gamma", "Kernel density estimate"), col = c("slateblue", "violetred"), lty = c(2,1), bty = "n")
title(xlab = expression(theta), ylab = "Density")
```

From the density histogram above, the density appears to be monotonically decreasing in the interval $(0,\infty)$ suggesting a shape parameter $\alpha = 1$. The vast majority of the data is less than 200, so we solve the following equation for $\beta$ taking $\alpha = 1$:
$$\begin{align*}
0.999 & = \int\limits_0^{200} \beta e^{-\beta\theta} d\theta \\
& = 1 - e^{-200\beta} \\
\implies \beta & = \frac{-1}{200} \log(0.001) \\
& = 0.035
\end{align*}$$
Therefore, the conjugate prior distribution of $\theta$ is:
$$p(\theta ~|~ \alpha = 1, \beta = 0.035) \propto e^{-(0.035)\cdot\theta}$$

### Posterior Distribution  

In general, the posterior distribution is Gamma$\left(\alpha + \sum\limits_{i=1}^n y_i, \beta + \sum\limits_{i=1}^n t_i\right)$. Thus, the posterior distribution of $\theta$ is Gamma(186, 18.683)
...
curve(dgamma(x, shape = alpha.post, rate = beta.post), from = 0, to = 50, n = 500, lty = 1, col = "violetred", ann = FALSE, bty = "n")
curve(dgamma(x, shape = alpha, rate = beta), from = 0, to = 50, n = 500, lty = 2, col = "slateblue", add = TRUE)
...
cat("The ", 100*attributes(hpd)$credMass, "% HPD Region for the kill rate per hour (theta) is: [", paste(round(hpd,2), collapse = ", "), "]", sep = "")
```
...
post.preds <- mapply(dnbinom, x = 0:60, MoreArgs = list(size = rep(alpha.post, length(t.new)), prob = (beta.ppd/(beta.ppd + 1))))
...
plot(1, 1, ann = FALSE, axes = FALSE, type = "n", lty = 3, col = "grey50", bty = "n", xlim = c(0, 60), ylim = c(0, 0.12))
for (i in 1:4) {
  points(0:60, post.preds[i, ], pch = 20, col = col.vec[i])
  lines(rep(x.modes[i] - 1, 2), c(-0.1, modes[i]), col = "grey50", lty = 3)
}
axis(1, seq.int(0,60,10), col = "gray50")
axis(2, seq.int(0,0.12,0.02), col = "gray50")
...
text(x.modes - 1, modes, labels = round(modes, 3), pos = 3, offset = 0.75)
...
```{r, echo = FALSE}
plot(qt$hours, qt$body.count, ann = FALSE, axes = FALSE, xlim = c(1.5, 3), ylim = c(0,70), col = "violetred", pch = 4)
#box(col = "gray50", lty = 1)
axis(1, at = seq.int(1.5, 3, 0.5), col = "gray50")
axis(2, at = seq.int(0,70,10), col = "gray50")
...
t.sim <- runif(m, min = min(qt$hours), max = max(qt$hours))
...
```
```{r, echo = FALSE}
hist(sim.min, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,40), ylim = c(0,0.15), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.min, 2), c(0, 0.12), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval, 2)), side = 3)
text(obs.min, 0.12,  obs.min, pos = 3, offset = 0.5)
```
...
```
```{r, echo = FALSE}
hist(sim.max, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,80), ylim = c(0, 0.12), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.max, 2), c(0, 0.10), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value =", round(pval, 2)), side = 3)
text(obs.max, 0.10, obs.max, pos = 3)
```
...
```{r}
obs.range <- diff(range(qt$body.count))
sim.itmd <- apply(yrep, 2, range)
sim.range <- apply(sim.itmd, 2, diff)
pval <- length(sim.range[sim.range <= obs.range]) / m
```
```{r, echo = FALSE}
hist(sim.range, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,80), ylim = c(0,0.12), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.range, 2), c(0, 0.10), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval, 2)), side = 3)
text(obs.range, 0.10, obs.range, pos = 3)
```
...
```{r}
obs.IQRange <- IQR(qt$body.count)
sim.IQRange <- apply(yrep, 2, IQR)
pval <- length(sim.IQRange[sim.IQRange <= obs.IQRange]) / m
```
```{r, echo = FALSE}
hist(sim.IQRange, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,30), ylim = c(0,0.2), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.IQRange, 2), c(0, 0.18), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval, 2)), side = 3)
text(obs.IQRange, 0.18, obs.IQRange, pos = 3)
```
...
```
```{r, echo = FALSE}
hist(sim.mean, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,50), ylim = c(0,0.12), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.mean, 2), c(0, 0.10), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval, 2)), side = 3)
text(obs.mean, 0.10, obs.mean, pos = 3)
```
...
```{r}
obs.median <- median(qt$body.count)    # observed median
sim.median <- apply(yrep, 2, median)   # simulated median
pval <- length(sim.median[sim.median >= obs.median]) / m
```
```{r, echo = FALSE}
hist(sim.median, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,50), ylim = c(0,0.12), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.median, 2), c(0, 0.10), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval, 2)), side = 3)
text(obs.median, 0.10, obs.median, pos = 3)
```
...
```{r}
obs.mad <- mad(qt$body.count)    # observed median
sim.mad <- apply(yrep, 2, mad)   # simulated median
pval <- length(sim.mad[sim.mad >= obs.mad]) / m
```
```{r, echo = FALSE}
hist(sim.mad, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,50), ylim = c(0,0.3), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.mad, 2), c(0, 0.275), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", round(pval, 2)), side = 3)
text(obs.mad, 0.275, round(obs.mad, 2), pos = 3)
```
...
```
```{r, echo = FALSE}
hist(sim.var, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,700), ylim = c(0,0.04), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.var, 2), c(0, 0.035), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", pval), side = 3)
text(obs.var, 0.035, round(obs.var, 2), pos = 3)
```
...
```
```{r, echo = FALSE}
hist(sim.ratio, breaks = 100, right = FALSE, freq = FALSE, ann = FALSE, xlim = c(0,80), ylim = c(0,0.5), col = "#1f78b4", border = "#a6cee3")
lines(rep(obs.ratio, 2), c(0, 0.45), col = "#e31a1c", lwd = 1.5)
mtext(paste("p-value:", pval), side = 3)
text(obs.ratio, 0.45, round(obs.ratio, 2), pos = 3)
```
 ...
 * Negative binomial as a robust alternative to poisson.
 * Poisson regression
 * Hierachichal model
